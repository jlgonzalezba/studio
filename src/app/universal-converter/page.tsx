...
const CONVERSION_DATA = {
  "Longitud": ["metro (m)", "kilómetro (km)", "centímetro (cm)", "milímetro (mm)", "micrómetro (µm)", "nanómetro (nm)", "pulgada (in)", "pie (ft)", "yarda (yd)", "milla (mi)", "milla náutica (nmi)"],
  "Masa": ["kilogramo (kg)", "gramo (g)", "miligramo (mg)", "microgramo (µg)", "tonelada métrica (t)", "tonelada corta EUA (ton US)", "libra (lb)", "onza (oz)"],
  "Tiempo": ["segundo (s)", "milisegundo (ms)", "microsegundo (µs)", "minuto (min)", "hora (h)", "día (d)", "semana (wk)"],
  "Corriente eléctrica": ["ampere (A)", "milliampere (mA)", "microampere (µA)", "kiloampere (kA)"],
  "Temperatura": ["Celsius (°C)", "Fahrenheit (°F)", "Kelvin (K)"],
  "Cantidad de sustancia": ["mol (mol)", "milimol (mmol)", "micromol (µmol)"],
  "Intensidad luminosa": ["candela (cd)", "millicandela (mcd)", "kilocandela (kcd)"],
};
...
export default function UniversalConverterPage() {
  ...
  const handleSwapUnits = () => {
    ...
    if (result && !error) setInputValue(result);
  };
  ...
  return (
    <main className="flex-grow container mx-auto px-4 py-8">
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
        ...
        {/* 6. Interfaz de conversión actualizada */}
